...
public class PatternValueTypeFactory extends PatternValueFactory {
    ...
    @Override
    public PatternValue getInstance(String pattern) {
        return new PatternValueType(pattern);
    }
    private final class PatternValueType extends PatternValue{

        public PatternValueType(String pattern) {
            super(pattern);
        }
        ...
        private boolean check(ValueType pattern, ValueType valueType) {
            return pattern == valueType || valueType.getParent() != null && check(pattern, valueType.getParent());
        }
